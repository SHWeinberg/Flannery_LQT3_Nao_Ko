...
function [time, X, parameters,beats_new] = modelRunner_Clancy( X0, options, parameters, beats, ignoreFirst)
...
% Cell type (endo/epi/mid) - default to endo
cellType = 0; if (isfield(parameters,'cellType')) cellType = parameters.cellType;end

% check steady-state
checkSS = 0; if (isfield(parameters,'checkSS')) checkSS = parameters.checkSS; end

% timestep
dt = 0.05; if (isfield(parameters,'dt')) dt = parameters.dt;end % CHANGED FROM 1
parameters.dt = dt; 
...
% Extracellular concentrations - change with parameter study
...
% Current multipliers - change with parameter study
...
Jrelnp_Multiplier = 1; if (isfield(parameters,'Jrelnp_Multiplier')) Jrelnp_Multiplier = parameters.Jrelnp_Multiplier; end
tauhL_Multiplier = 1; if (isfield(parameters,'tauhL_Multiplier')) tauhL_Multiplier = parameters.tauhL_Multiplier; end
...
% Stimulus parameters
stimAmp = -56; if (isfield(parameters,'stimAmp')) stimAmp = parameters.stimAmp; end 
stimDur = 2; if (isfield(parameters,'stimDur')) stimDur = parameters.stimDur; end %%%%%% CHANGED to 2 ms fomr 1 ms
...
if(~isfield(parameters,'model'))
    parameters.model = @model_Torord_Clancy;
end
...
for n=1:beats
    if (verbose)
       % disp(['Beat = ' num2str(n)]); % THIS IS USUALLY NOT COMMENTED OUT
    end
    ...
    [time{n}, X{n}]=ode15s(parameters.model,0:dt:CL,X0,options,1,  cellType, Jleak_Multiplier, tauhL_Multiplier, Jrelnp_Multiplier, CaMKa_Multiplier, ICaL_Multiplier, ...
        ...
        INaK_Multiplier, INab_Multiplier, ICab_Multiplier, IpCa_Multiplier, ICaCl_Multiplier, IClb_Multiplier, Jrel_Multiplier,Jup_Multiplier,nao,cao,ko,ICaL_fractionSS,INaCa_fractionSS, stimAmp, stimDur, parameters.mt1, parameters.mt2, parameters.mf1, parameters.mf2, vcParameters, apClamp, extraParams);
...
APD901 =  APD90fun_1Beat(time{n},X{n-100}(:,1),dt,CL);
APD902 =  APD90fun_1Beat(time{n},X{n}(:,1),dt,CL);
CaTA1 =  max(X{n-100}(:,6));
CaTA2 =  max(X{n}(:,6));
SS_Check = SSCheckFun(APD901, APD902, CaTA1, CaTA2); % check for steady-state

% Simulate additional beats based on steady-state criteria - assuming
% constant iso 
while SS_Check == 0  % while criteria is not met, continue
    n = n+1; 
    ...
    X0=X{n}(size(X{n},1),:); % set end state from previous beat as the starting state of the next beat

    % Run SS_Check function
    APD901 =  APD90fun_1Beat(time{n},X{n-100}(:,1),dt,CL);
    APD902 =  APD90fun_1Beat(time{n},X{n}(:,1),dt,CL);
    CaTA1 =  max(X{n-100}(:,6));
    CaTA2 =  max(X{n}(:,6));
    ...
    % add contingency, if paced for 5000, assume no steady-state and end
    % while loop. 
    if beats == 1000
        break
    end
...
beats_new = length(X); % output number of beats
end
%% Beats we're not interested in are deleted.
ignoreFirst = []; % update index of final beat - ADJUSTED FOR BENCHMARKING, RETURN TO N - 5
time(1:ignoreFirst) = [];
X(1:ignoreFirst) = [];
